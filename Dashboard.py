...
def create_trade_chart(df, trade_label, show_all_exits=False, market_hours_only=False):
    ...
    # Create subplots: main chart with secondary y-axis, and EWO/RSI subplot below
    if has_ewo:
        fig = make_subplots(
            rows=2, cols=1,
            shared_xaxes=True,
            ...
            specs=[[{"secondary_y": True}], [{"secondary_y": True}]]  # Secondary y for RSI on row 2
        )
    else:
    ...
    if has_ewo:
        ...
        # RSI on secondary y-axis (right side) - white color
        if has_rsi:
            # RSI line
            fig.add_trace(
                go.Scatter(
